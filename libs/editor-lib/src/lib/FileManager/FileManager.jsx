...
import PopupDialog from './Elements/PopupDialog1';
...
import { Paper } from '@koj-react/ui/index';
import { Drawer, Button, Radio, Space, Table } from 'antd';
...
function FileManager(props) {
  ...
  height =
    typeof height !== 'string'
      ? typeof height !== 'undefined' || height > 300
        ? `${height}px`
        : '300px'
      : height;

  const bigHeight = `${window.innerHeight - 200}px`;
  ...
  const operations = {
    ...
    handlePaste: () => {
      let files = props.bufferedItems.files.map((item) =>
        pathWithoutDomain(item.path)
      );
    ...
    handleDelete: () => {
      let files = props.selectedFiles.map((item) =>
        pathWithoutDomain(item.path)
      );
    ...
    handlePreview: () => {
      ...
      handleClickPopupOpen({
        ...
        description: `<img style="width: 100%" src="${mainconfig.serverPath}${file.path}" />`,
        handleClose: handleClose,
        nameInputSets: {},
      });
      // Emitter.emit('insert_image', `${mainconfig.serverPath}${file.path}`);
    },
      ...
      try {
        ...
        if (destination !== undefined && files.length !== 0) {
          ...
          props.selectedFiles.forEach((f) => {
            if (f.path !== files[0]) {
              filesMove.push(f.path);
            }
          });
          filesMove.push(...files);
          props
            .pasteFiles(filesMove, 'cut', destination)
            .then((result) => {
              operations.handleReload();
              props.unsetSelectedFiles();
              // setMessages([
  ...
  const checkSelectedFileType = (type) => {
    try {
      switch (type) {
        ...
        case 'file':
          return props.selectedFiles[0].type === 'file' || 'image';
  ...
  const allButtons = {
    ...
    choose: {
      title: 'Choose',
      icon: 'icon-layers',
      onClick: operations.handleChoose,
      disable: !(
  ...
  useEffect(() => {
    ...
    // console.log('divRef',divRef.current.width);
    setWidth(divRef.current.offsetWidth);
    window.addEventListener('resize', () => {
      if (!!divRef?.current?.offsetWidth) setWidth(divRef.current.offsetWidth);
    });
    () => {
  ...
  return (
    <div>
      ...
      <div className={expand ? classes.fmExpanded : classes.fmMinimized}>
        <Paper id="file_manager_modal" ref={divRef}>
          ...
          <TopBar
            ...
            isVisibleButtonDrawer={width > 900 ? false : true}
            screenWidth={width}
          />
          <Grid container>
            {width > 900 ? (
              <Grid item xs={2} className={classes.folderSide}>
                <PerfectScrollbar>
                  <div
                    style={{
                      maxHeight: expand ? bigHeight : height,
                      height: expand ? bigHeight : height,
                      backgroundColor: '#fff',
                    }}
                  >
                    <FolderBar
            ...
            >
              <PerfectScrollbar>
                ...
                >
                  ...
                  />
                  {/* <ListViewAnt filesList={props.filesList} /> */}
                </div>
              </PerfectScrollbar>
